...
function userlogin() {
	...
	$res = SQL_Query_exec("SELECT * FROM (SELECT * FROM users INNER JOIN groups ON users.class=groups.group_id WHERE id=$_COOKIE[uid] AND users.enabled='yes' AND users.status = 'confirmed') x join (SELECT COUNT(*) invited_users FROM users WHERE invited_by=$_COOKIE[uid]) z");
...
function logincookie($id, $password, $secret, $updatedb = 1, $expires = 9999999999 ){ //0x7fffffff) {
...
function format_urls($s){
    ...
    "\\1<a href='http://www.redirect.am/?\\2' target='_blank'>\\2</a>", $s);
}
...
function format_comment($text) {
	...
	$s = preg_replace(
		...
		"<a href='http://www.redirect.am/?\\1' target='_blank'>\\3</a>", $s);

	// [url]http://www.example.com[/url]
	$s = preg_replace(
		"/\[url\]((http|ftp|https|ftps|irc):\/\/[^<>\s]+?)\[\/url\]/i",
		"<a href='http://www.redirect.am/?\\1' target='_blank'>\\1</a>", $s);
